...
def single_page_view(request, search=None):
    if search == None:
        ...
        listings = listing.objects.annotate(search=SearchVector('title', 'description', 'price', 'email', 'public_token')).filter(search=search)

    form = ListingForm()
    return render(request, 'single_page/single_page.html', {'form':form, 'listings':listings})
